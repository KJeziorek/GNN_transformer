...
def _crop_events(data, left, right, not_crop_idx=None):
    if not_crop_idx is None:
        not_crop_idx = torch.all((data.pos[:,:2] >= left) & (data.pos[:,:2] <= right), dim=1)
...
class RandomHFlip(BaseTransform):
    def __init__(self, p: float, width=240, height=180):
        self.p = p
        self.width = width
        self.height = height
    ...
    def __call__(self, data: Data):
        ...
        data.pos[:,0] = self.width - 1 - data.pos[:,0]

        if hasattr(data, "bbox"):
            data.bbox[:, 0] = self.width - 1 - (data.bbox[:, 0] + data.bbox[:, 2])
...
class Crop(BaseTransform):
    ...
    """
    def __init__(self, min: List[float], max: List[float], width=240, height=180):
...
class RandomZoom(BaseTransform):
    def __init__(self, zoom, subsample=True, width=240, height=180):
        self.zoom = zoom
...
class RandomCrop(BaseTransform):
    ...
    """
    def __init__(self, size: List[float] = [0.75, 0.75], dim: List[int]=[0,1], p=0.5, width=240, height=180):
...
class RandomTranslate(BaseTransform):
    ...
    """
    def __init__(self, size: List[float], width=240, height=180):
