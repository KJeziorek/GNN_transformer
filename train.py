...
def main(args):
    ...
    model = LNDetection(lr=1e-2, weight_decay=1e-5, num_classes=dm.num_classes, batch_size=args.batch_size)
    wandb_logger = WandbLogger(project='event_detection', name=f'{args.dataset}_{args.radius}', log_model='all')
    ...
    # trainer.fit(model, dm, ckpt_path='event_detection/387fwdex/checkpoints/epoch=619-step=64480.ckpt')
